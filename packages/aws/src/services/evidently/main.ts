...
export class EvidentlyManager {
  ...
  public createProject(id: string, scope: CommonConstruct, props: EvidentlyProjectProps) {
    if (!props) throw new Error(`EvidentlyProject props undefined for ${id}`)
    if (!props.name) throw new Error(`EvidentlyProject name undefined for ${id}`)
  ...
  public createFeature(id: string, scope: CommonConstruct, props: EvidentlyFeatureProps) {
    if (!props) throw new Error(`EvidentlyFeature props undefined for ${id}`)
  ...
  public createLaunch(id: string, scope: CommonConstruct, props: EvidentlyLaunchProps) {
    if (!props) throw new Error(`EvidentlyLaunch props undefined for ${id}`)
    if (!props.name) throw new Error(`EvidentlyLaunch name undefined for ${id}`)
  ...
  public createExperiment(id: string, scope: CommonConstruct, props: EvidentlyExperimentProps) {
    if (!props) throw new Error(`EvidentlyExperiment props undefined for ${id}`)
    if (!props.name) throw new Error(`EvidentlyExperiment name undefined for ${id}`)
  ...
  public createSegment(id: string, scope: CommonConstruct, props: EvidentlySegmentProps) {
    if (!props) throw new Error(`EvidentlySegment props undefined for ${id}`)
    if (!props.name) throw new Error(`EvidentlySegment name undefined for ${id}`)

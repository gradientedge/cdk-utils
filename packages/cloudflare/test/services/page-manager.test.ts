...
const testStackProps: any = {
  ...
  extraContexts: [
    ...
    'packages/cloudflare/test/common/config/pages.json',
  ],
  features: {},
...
describe('TestCloudflarePagesManager', () => {
  ...
  test('provisions pages project as expected', async () => {
    await outputToPromise(
      pulumi
        ...
        .apply(([id, urn, accountId, name, productionBranch]) => {
          ...
          expect(urn).toEqual(
            'urn:pulumi:stack::project::construct:test-common-stack$cloudflare:index/pagesProject:PagesProject::test-pages-project-dev'
          )
    ...
    await outputToPromise(
      pulumi
        ...
        .apply(([id, urn, accountId, name, productionBranch, buildConfig, deploymentConfigs]) => {
          ...
          expect(urn).toEqual(
            'urn:pulumi:stack::project::construct:test-common-stack$cloudflare:index/pagesProject:PagesProject::test-pages-project-with-build-dev'
          )
          expect(accountId).toEqual('123456789012')
...
describe('TestCloudflarePagesManager', () => {
  ...
  test('provisions pages domain as expected', async () => {
    await outputToPromise(
      pulumi
        .all([
        ...
        .apply(([id, urn, accountId, name, projectName]) => {
          ...
          expect(urn).toEqual(
            'urn:pulumi:stack::project::construct:test-common-stack$cloudflare:index/pagesDomain:PagesDomain::test-pages-domain-dev'
          )
          expect(accountId).toEqual('123456789012')
...
describe('TestCloudflarePagesManager', () => {
  ...
  test('provisions page rule as expected', async () => {
    await outputToPromise(
      pulumi
        .all([
        ...
        .apply(([id, urn, actions, priority, target, zoneId]) => {
          ...
          expect(urn).toEqual(
            'urn:pulumi:stack::project::construct:test-common-stack$cloudflare:index/pageRule:PageRule::test-page-rule-dev'
          )
...
describe('TestCloudflarePagesManager - With explicit zoneId', () => {
  test('provisions page rule with explicit zoneId', async () => {
